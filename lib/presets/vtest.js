...
class vtestPreset extends _defs.IPresetAddressing {
  constructor(...args) {
    super(...args);
  ...
  static async onCache({
    ...
    const uuid = getUUIDBuffer(id);
    setInterval(() => vtestPreset.updateAuthCache(uuid, store), 1e3);
    vtestPreset.updateAuthCache(uuid, store);
  }
